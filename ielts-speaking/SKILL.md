...
name: ielts-speaking
description: |
  雅思口语素材工厂 v3.0。话题分组 + 万能故事生成 + Part 3 追问预测 + 高分表达 + 素材存档。
...
1. 读取 `~/.ielts/speaking/stories/` 下所有 `.md` 文件
2. 统计已准备的素材数量和覆盖的万能故事组
3. 显示：「已有 {n} 个口语素材，覆盖 {x}/5 组万能故事」
...
生成素材后，写入文件：`~/.ielts/speaking/stories/YYYY-MM-DD-{topic}.md`
...
在 `~/.ielts/stats/progress.md` 的「口语」表格中追加一行：
```
| YYYY-MM-DD | {话题} | {组号} |
```
