...
### Phase 5.5：复习计划生成（核心闭环）

批改报告输出后，**必须**生成个性化复习材料。这是提分的关键环节。

#### 5.5.1 写作建议总结

基于当前分数和目标分数，给出**具体可执行**的建议（不说空话）：

```markdown
## 写作建议（目标：{当前分数+0.5~1} 分）

### 你的薄弱项
1. **{维度名}**（{当前分}→{目标分}）：{一句话描述差距}
2. **{维度名}**（{当前分}→{目标分}）：{一句话描述差距}
...
将改写版翻译成中文，让用户翻译回英文。选择改写版中的核心句子（5-8 句）：

```markdown
## 练习 A：中文翻译

将以下中文翻译成英文，然后对比改写版原文。

### 开头段
1. 中文：{改写版句子的中文翻译}
   
   你的翻译：_______________________
...
### 原文
"{改写版段落，挖掉 5-8 个关键词，用 ______（数字）代替}"
...
1. **今天**：完成练习 A（中文翻译）
2. **明天**：完成练习 B（挖空填词）+ 背诵词汇卡
3. **后天**：完成练习 C（默写）+ 重写作文
4. 重写后发给我，我对比这次和上次的进步
