...
name: ielts-writing
description: |
  雅思写作批改教练 v3.0。四维评分 + 句子级标注 + 改写对比 + 审题检查 + 数据持久化。
  触发方式：/ielts-writing、「批改作文」「帮我看看这篇」「审题」「写作练习」
metadata:
  version: 3.0.0
---

# IELTS Writing — 雅思写作批改教练 v3.0
...
| **批改模式** | 用户给了题目 + 作文 | 四维评分 + 句子级标注 + 改写对比 + 数据存档 |
...
2. 读取 `~/.ielts/writing/essays/` 下所有 `.md` 文件 → 计算历史趋势
3. 统计：
   - 历史总篇数
   - 各维度分数走势（最近 5 篇）
   - 高频错误类型 Top 3
...
写入文件：`~/.ielts/writing/essays/YYYY-MM-DD-task{X}-NNN.md`
...
将批改中发现的同义替换追加到 `~/.ielts/writing/synonym-library.md`（如不存在则创建）和 `~/.ielts/vocabulary/synonym-master.md`。

追加格式（在表格末尾添加行）：
```
| {原文用词} | {高分替换} | writing | YYYY-MM-DD |
```
...
在 `~/.ielts/stats/progress.md` 的「写作」表格中追加一行：
```
| YYYY-MM-DD | Task{X} | {tr} | {cc} | {lr} | {gra} | {overall} | {target} |
```

#### 6.4 更新错题本

将本次发现的错误追加到 `~/.ielts/errors/error-book.md`：
- 更新 frontmatter 中的 `total_errors` 和 `last_updated`
- 在表格中追加新行或更新已有错误类型的频率
